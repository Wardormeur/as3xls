package com.as3xls.xls {
	...
	public class ExcelFile {
		...
		public function saveToByteArray():ByteArray {
			...
			bof.data.writeByte(0x10); // type of document ? 
			...
			for(var r:uint = 0; r < s.rows; r++) {
				for(var c:uint = 0; c < s.cols; c++) {
					var true_cell:Cell = s.getCell(r,c);
					var value:* = true_cell.value;
					...
					if(value is Date) {
					...
					}else if( true_cell.formula != null){
						
						cell.type = Type.FORMULA;	
						cell.data.writeByte(true_cell.formula.myRow);
						cell.data.writeByte(true_cell.formula.myCol);
						
						// Cell attributes
						//if(version == BIFFVersion.BIFF2) { //WE FORCED IT BEFORE
						...
						//alwaysrecalculate
						//if(version == BIFFVersion.BIFF2) { //WE FORCED IT BEFORE
							//Set it as false by default, since i cannot get it anymore
							cell.data.writeByte(0);
						//}
						...
						//if(version == BIFFVersion.BIFF2) { //WE FORCED IT BEFORE
							 cell.data.writeByte(true_cell.sharedTokens.length);
					//	}
						cell.data.writeBytes(true_cell.sharedTokens);
		...
		private function label(r:Record, s:Sheet):void {
			...
			var fmt:String;
			if(s.formats.length>0 && s.xformats.length>0){
				fmt = s.formats[s.xformats[indexToXF].format];
